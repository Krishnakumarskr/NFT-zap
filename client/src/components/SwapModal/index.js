...
const SwapModal = (props) => {
  ...
  const initSwap = async () => {
    ...
    const res = await nftInstance.methods
      .approve(swapAddress, parseInt(myNftData.tokenID))
      .send({ from: wallet.address });

    console.log(res);
    const amount = wallet.web3.utils.toWei("0.00069", "ether");
    const result = await wallet.ZAP.methods
      .newContract(
        props.nft.owner,
        "password",
        myNftData.contractAddress,
        parseInt(myNftData.tokenID),
        ...
        <Container className={"selection-container"}>
          <Row className={"nft-sticky-row"}>
            ...
            <div className="text-center">
              <Button className="zap-button" onClick={initSwap}>
                ZAP IT
              </Button>
            </div>
