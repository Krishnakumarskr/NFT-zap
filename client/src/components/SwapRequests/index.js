...
const SwapRequests = () => {
  ...
  const zapWithdraw = async (data) => {
    ...
    const swapAddress = "0x27a2569cD335AC10e3D34dA67f1094ca0632F6D9";
    const nftInstance = new wallet.web3.eth.Contract(
      ERC721.abi,
      data.requestedcontract
    );

    const approveres = await nftInstance.methods
      .approve(swapAddress, parseInt(data.requestedid))
      .send({ from: address });
    console.log(approveres);

    const res = await ZAP.methods
      .withdraw(data.contractID, data.preimage)
      .send({ value: amount, from: address });
    console.log(res);
  };
    ...
    const fetchRequestData = async () => {
      ...
      for (let i = totalContracts; i >= 0; i--) {
        ...
        if (contract.sender === address || contract.receiver === address) {
          ...
          try {
            contract.tokenURI = await instance.methods
              .tokenURI(parseInt(contract.tokenId))
              .call();
            const response = await axios.get(contract.tokenURI);

            contract.requestedURI = await requestedInstance.methods
              .tokenURI(parseInt(contract.requestedid))
              .call();
                  ...
                  <Col md={6} className="col text-left">
                    <div className="img-container text-left">
                      <img
                        ...
                        alt=""
                        className="request-list-img"
                      />
                      <img
                        src={data.requestedMetaData.image}
                        alt=""
                        className="request-list-img"
                      />
                    </div>
              ...
              return (
                <Row className="request-list" key={i}>
                  ...
                  <Col md={6} className="col button-col text-right">
                    <div className="btn-container text-right">
                      <p>
                        <button className="btn btn-danger">Refunded</button>{" "}
                      </p>
                      {/* <button onClick={() => zapWithdraw(data)} className="btn btn-primary">ZAP IT</button> */}
                    </div>
                  </Col>
